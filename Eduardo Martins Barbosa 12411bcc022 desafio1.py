...
# Eduardo Martins Barbosa - 12411BCC022
class Submissao:
    DIM_MAX = DIM_MAX
    ...
    def fit(self, X: torch.Tensor) -> None:
        ...
        explicit_dim = 2 * d + len(pares)          
        if explicit_dim > self.DIM_MAX - 1:
            pares = []                              
            explicit_dim = 2 * d
        self.pares = pares
        self.d2 = (d == 2)
        ...
        restante = max(self.DIM_MAX - explicit_dim, 0)
        self.Ws, self.bs = [], []
        if restante > 0:
            g = torch.Generator().manual_seed(0)    
            n_sub = min(300, len(aug))
            d2mat = torch.cdist(aug[:n_sub], aug[:n_sub]) ** 2
            pos = d2mat[d2mat > 0]
            sigma0 = math.sqrt(pos.median().item() / 2) if len(pos) > 0 else 1.0
            sigma0 = max(sigma0, 1e-3)
            escalas = [sigma0 * f for f in (0.15, 0.4, 1.0, 2.5)]   
            base = restante // len(escalas)
            for i, sc in enumerate(escalas):
                n_cols = base if i < len(escalas) - 1 else restante - base * (len(escalas) - 1)
                if n_cols <= 0:
                    continue
                self.Ws.append(torch.randn(aug.shape[1], n_cols, generator=g) / sc)
                self.bs.append(torch.rand(n_cols, generator=g) * 2 * math.pi)
 
    def _aug(self, Xs: torch.Tensor) -> torch.Tensor:
        if not self.d2:
            return Xs
        r = torch.linalg.norm(Xs, dim=1, keepdim=True)
        theta = torch.atan2(Xs[:, 1], Xs[:, 0]).unsqueeze(1)
        if not hasattr(self, "r_mu"):
            self.r_mu, self.r_sd = r.mean(), r.std() + 1e-8
        return torch.cat([Xs, (r - self.r_mu) / self.r_sd, theta / math.pi], dim=1)
 
    def phi(self, X: torch.Tensor) -> torch.Tensor:
        Xs = (X - self.mu) / self.sd
        partes = [Xs, Xs ** 2]
        if self.pares:
            prod = torch.stack([Xs[:, i] * Xs[:, j] for i, j in self.pares], dim=1)
            partes.append(prod)
        aug = self._aug(Xs)
        for W, b in zip(self.Ws, self.bs):
            partes.append(math.sqrt(2 / W.shape[1]) * torch.cos(aug @ W + b))
